...
const isValidToken = async (token) => {
  return db.AccessLog.findOne({
    where: {
      accessToken: token,
      logoutAt: { [db.Sequelize.Op.eq]: null }
    }
  })
}
...
const authorize = (permissionSlugs = [], option = 'every') => {
  return async (req, res, next) => {
    ...
    const { id } = req.user
    const getPermissionByUserId = await UserService.getUseById({ id })

    const permissions = getPermissionByUserId?.data?.role?.permissions.map((item) => item.permissionSlug) || []
    ...
    if (!isValidPermission) {
      return next(new ForbiddenException('Bạn không có quyền truy cập vào tài nguyên này'))
    }
